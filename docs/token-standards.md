...
- `Ownable.sol` / `AccessControl.sol` → RBAC; this prototype inherits **`Ownable`** (owner = Curator).
...
- **If it were tokenized:** each `Dataset` is unique → a natural **ERC-721** (one NFT per certificate, `tokenId = id`), with the Curator as minter and `Status` as on-chain metadata. The current design keeps it as a lean registry to avoid NFT transfer-market surface that the use case doesn't need.
