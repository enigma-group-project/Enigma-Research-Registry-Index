...
document.getElementById("verify").onclick = async () => {
  // TODO(member3): const [valid,status,owner,issuer] = await readContracts().verification.verifyDataset(recordId(val("key")), hashBytes(val("artifact")));
  //               out((valid?"VALID":"NOT VALID") + ... STATUS[Number(status)] ...);
  out("TODO(member3): implement verifyDataset call");
};
