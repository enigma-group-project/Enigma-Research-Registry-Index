# Security Assurance — Enigma-Research-Registry-Index
...
| **[M] Mechanical** | Reentrancy protection on state-changers; access control enforced; CEI ordering; no unchecked external-call return; events on state change. | ✅ OZ `ReentrancyGuard` on `registerDataset` + AuditTrail mutators; OZ `Ownable` (Curator) + Publisher role; CEI; events on every mutation. |
...
- **Design & implement:** chose a registry/attestation model (not a token); built from reviewed OZ components (`Ownable`, `ReentrancyGuard`, `Pausable`); defined roles (Curator, Publisher, Owner), state (`Status`), and transfer/revoke/renew rules.
- **Harden & verify:** enforce access control (themed `onlyCurator`/`onlyTestCenter`/`onlyController`); preserve state invariants; protect external interactions (CEI + guard); test edge cases + fuzz + invariants.
...
|---|---|---|
| Reentrancy (SC05) | `registerDataset`, AuditTrail mutators | OZ `ReentrancyGuard` + CEI (effects before any external call) |
| Broken access control (SC01) | issue / setController / revoke / pause | `onlyTestCenter` / `onlyCurator` / `onlyController`; OZ `Ownable` for Curator |
| Unauthorized state change | `controller*` mutators | `onlyController`; controller is set once (`ControllerSet`) by the Curator |
| Fake issuer | `registerDataset` | gated by `IssuerRegistry.isPublisher` (Curator-granted) |
| Replay / duplicate record | `registerDataset` | `AlreadyExists` check on the record id |
| Denial via stuck state | issuance | `Pausable` lets the Curator halt issuance during an incident, then `unpause` |
| Front-running | `revokeDataset` | issuer/Curator-only; gas price does not change correctness |
